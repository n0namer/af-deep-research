...
- 2026-09-01 CURRENT capability note: ADR/design decision below is ACCEPTED, but this execution cannot independently prove the loaded `/app` profile implementation because the authoritative VPS Terminal currently returns `no available server`, while the DEV typed live-patch/read lane rejects this target (`live_patch_not_allowed`). Treat any previously recorded profile hashes/tests as historical accepted evidence until fresh `/app` readback + targeted regression is available; do not mutate blindly or route application code through GitHub/redeploy to compensate.
