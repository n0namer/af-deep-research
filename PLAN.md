...
- live semantic false-premise anchor is running server-side as `run_1788252070390_c753d4aa`. HTTP client intentionally disconnected after 3s, but durable ResearchRun exists with `stage=started`, `status=running`, proving client lifetime is not the evaluation lifetime. No semantic PASS/FAIL is claimed yet;
