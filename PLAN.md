...
20. BLOCKED_RUNTIME_LOAD — the ResearchHypothesis source fix is complete and offline-validated but not yet loaded into PID1. A typed same-container reload attempt is currently blocked by enforced `required_context:context_check_unavailable`; advisory guidance is `ALLOW` but Required Context remains degraded. Do not bypass this with deploy/redeploy/PR/CI, opaque process mutation or GitHub application-code transport. Next move: after fresh Required Context/readback allows reload, load the same container, run a focused loaded `generate_adaptive_hypothesis` BLS canary, then repeat `DR-L04` before moving to `DR-L05`.
