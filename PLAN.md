...
- CURRENT deterministic validation: `py_compile` PASS for eval/ResearchRun/pipeline/tests; 9 evaluation regressions + 5 ResearchRun regressions = `14 PASS` via direct execution. Runtime image still lacks pytest, so canonical pytest remains a validation-environment blocker;
