...
17. BLOCKED_RUNTIME_LOAD — source fix is validated but not yet loaded into PID1: same-container `reloadTarget` is temporarily blocked by `required_context:context_check_unavailable`. Do not bypass that mediation with deploy/redeploy/PR/CI or opaque process mutation. After a fresh Required Context/readback allows reload, load the same container, run a focused inquiry-probe canary, then repeat `DR-L04` before starting `DR-L05`.
