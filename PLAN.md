...
- fresh-process patched canary proved `STRUCTURED_LIMIT 2` was loaded, but failed before reaching relationship extraction: `generate_adaptive_search_streams` first timed out after 90s, then fresh-pool retry returned `401 invalid API key`. Runtime env still has a non-empty `OPENAI_API_KEY`, `OLLAMA_BASE_URL=https://api.proxy.gonka.gg/v1`, and model `openai/deepseek-ai/DeepSeek-V4-Flash-0731`; therefore this is not simply a missing env key. Current suspicion is retry/provider/config-path instability after fresh-pool reset or external generation/config churn;
- do not continue semantic A1/A3 runs while the authoritative DEV generation is being externally replaced. First stabilize runtime identity long enough for one bounded canary and localize the retry-401 path; no infrastructure mutation is authorized by this plan entry.
