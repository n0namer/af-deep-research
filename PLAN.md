...
## Current 30-minute batch — ResearchRun replay proof + exact A1 recovery
...
- CURRENT deterministic validation: `py_compile` PASS for eval/ResearchRun/pipeline/tests; 9 evaluation regressions + 4 ResearchRun regressions = `13 PASS` via direct execution. Runtime image still lacks pytest, so canonical pytest remains a validation-environment blocker;
