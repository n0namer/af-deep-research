...
21. BLOCKED_EXECUTION_ROUTE — post-reload semantic verification is blocked outside Deep Research by the existing AgentField Actions gateway, not by the application source. Gateway health itself is green, while capability discovery/node inventory/async reasoner dispatch return `Bad Gateway`. Read-only inspection of the running gateway container localized the cause: its process command hard-codes obsolete upstream hostname `control-plane-edshqtkwskg3lrczekhcmd71-084728200638`, while CURRENT control-plane container is `control-plane-edshqtkwskg3lrczekhcmd71-115125416151`; gateway logs show DNS `socket.gaierror: [Errno -3] Try again`. Do not mutate/redeploy the shared gateway as part of Lane A without explicit scope approval. Next bounded move remains one loaded BLS `generate_adaptive_hypothesis` canary followed immediately by exact `DR-L04` once an already-authorized DEV execution route is available. No `DR-L05` until that evidence exists.
