...
- The new typed live-patch lane is real and partially working: `AGENTS.md` is present and requires stale-safe live patch -> check -> same-runtime reload -> canary; `/app/ERRORS.md` is absent and was not recreated. SourceLoop target `agentfield-dev-deep-research` is healthy and `/app/.source-commit` matches `936d4b22...`. An import-only prerequisite patch (`math`, `re`) was applied successfully through typed `fileAction`, capture id `vtchg_657dcbd437664eaba273b352bc4b2f16`, with readback SHA `125deca1...`.
- Both remaining exact restoration patches (bounded `ResearchQualityScore` + fail-closed `_parse_research_quality_text`, and plain-text `assess_research_completeness` transport) dry-run successfully against CURRENT `main.py`. Their APPLY is currently blocked by enforced operator mediation because Required Context is degraded with `required_context:context_check_unavailable`; do not bypass this guardrail with generic terminal/GitHub-code transport. Resume the same stale-safe patches only after a fresh Required Context check returns OK.
- Validator drift remains: target exposes `pytest_q`, but CURRENT runtime returns `/usr/local/bin/python3: No module named pytest`. Classify this as `VALIDATION_BLOCKER`, not product regression. Do not claim pytest PASS until the runner/dependency is actually present.
