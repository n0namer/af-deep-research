...
- retry/SDK hypothesis is now ruled out as the primary cause: a direct OpenAI-compatible POST to `LLM_ENDPOINT_GONKA` using the runtime `OPENAI_API_KEY` (without AgentField/LiteLLM) returns `HTTP 401 {"error":"invalid API key"}`. The runtime key is non-empty, `sk-` shaped, length 51, and not a known placeholder. Compose wiring proves `OPENAI_API_KEY=${LLM_KEY_GONKA}` and `OLLAMA_BASE_URL=${LLM_ENDPOINT_GONKA}`;
- minimal classifier re-probe after the canary also returns 401, confirming the credential failure is global for new Gonka AI calls, not specific to entity dedup or relationship extraction. Search/Tavily remains functional;
- repository search finds `LLM_KEY_GONKA` only as deployment input in AgentField compose files; no self-service key rotation/provisioning mechanism exists in the current source. Therefore the exact unblock criterion is: provide/restore a Gonka credential accepted by the configured proxy, then rerun the same false-premise canary under a stable generation;
- no valid alternate credential was read or exposed, no provider was silently substituted, and no application restart/redeploy/build was used. The temporary debug freeze must be rolled back to the pre-debug Coolify state (`git_branch=main`, same `git_commit_sha=8d852d1...`, auto-deploy=false) after this diagnostic batch;
- semantic A1/A3 live acceptance is BLOCKED on external Gonka credential validity. Container-first source validation and frozen/offline evaluation remain usable while this external credential is repaired.
