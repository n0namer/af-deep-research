...
- 2026-09-01 CURRENT runtime proof refreshed after terminal recovery: exact `/app` readback shows `DR_EVAL_PROFILE=semantic` as default; semantic profile uses `DR_SEMANTIC_TRANSPORT_TIMEOUT_SECONDS` default `1800s` only as dead-socket protection, preserves AgentField retries, and disables project-level decomposition/classification/search-planning/evidence-extraction timeout substitutions by passing `timeout=None`; resilience profile alone retains the short fallback budgets. `provider_telemetry.py` is present and wired into bootstrap + ResearchRun checkpoints. `py_compile` PASS; 5 semantic/premise/delivery regressions PASS; 3 profile/telemetry regressions PASS. Live anchor `run_1788254812484_313ef272` / execution `exec_1788254812549_ccfd9438` is currently active under `PROFILE semantic`, `LLM_TIMEOUT 1800.0`, structured/evidence concurrency `1/2`; no semantic verdict yet and no intervention is justified by elapsed time alone.
- CURRENT long-run observability finding from that anchor: the one-shot Python process remains alive (`/proc` state `S`, wait channel `ep_poll`) with established local AgentField sockets and an established external HTTPS socket, so evidence is consistent with provider/network wait rather than local CPU spin/deadlock. However `/e2e/deep-research-runs/run_1788254812484_313ef272.json` does not yet exist because `bootstrap.py` performs `decompose_answer_contract()` before `execute_verified_pipeline()` calls `begin_research_run()`, and provider telemetry records only terminal success/error events. This is an observability/durability defect, not a semantic failure. Next source increment after the active anchor completes: create durable `ResearchRun` before the first provider call and emit safe `provider.call.started` plus terminal events so an in-flight long provider call is visible without prompt/response/key data.
